...
export default function CountryTimeZoneFields({
  ...
  return (
    <div className="grid grid-cols-2 gap-4">
      <div>
        <label className="block text-base font-medium text-ink-2">Country</label>
        <select
          required
          value={country}
      ...
      <div>
        <label className="block text-base font-medium text-ink-2">Timezone</label>
        <select
          required
          value={timeZone}
          disabled={locked}
          onChange={(e) => onTimeZoneChange(e.target.value)}
          className="focus-ring mt-1 w-full border border-line bg-white px-3 py-2 text-ink disabled:bg-paper-2 disabled:text-ink-2"
        >
          <option value="" disabled>
            Select timezone
          </option>
