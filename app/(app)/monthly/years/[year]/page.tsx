...
export default async function ArchivedYearPage({
  ...
  return (
    <section className="mx-auto max-w-5xl px-6 py-10">
      <h1 className="text-2xl font-black tracking-tight text-slate-900 dark:text-white">{year}</h1>
      <p className="mt-2 text-sm text-slate-500 dark:text-slate-400">Read-only — fully archived.</p>

      {monthsWithData.length === 0 ? (
        <p className="mt-8 text-sm text-slate-500 dark:text-slate-400">No entries recorded for {year}.</p>
      ) : (
        <div className="mt-8">
          <MonthlyHistoryStack
