    ...
    <main className="min-h-screen bg-paper">
      <header className="border-b border-line bg-white">
        <div className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4">
          <div>
            <p className="font-display text-lg italic text-ink">WealthBridge</p>
            <p className="text-xs text-ink-2">{household.name}</p>
          </div>
          <div className="flex items-center gap-4">
            <nav className="flex items-center gap-4 text-sm text-ink-2">
              <Link href="/accounts" className="hover:text-span">
                Investments
              </Link>
              <Link href="/goals" className="hover:text-span">
                Goals
              </Link>
              <Link href="/allocation" className="hover:text-span">
                Targets
              </Link>
              {session.isPlatformOwner && (
                <Link href="/platform" className="hover:text-span">
                  Clients
                </Link>
              )}
            </nav>
            <span className="hidden text-sm text-ink-2 sm:inline">
              {session.email} · <span className="text-span">{session.role}</span>
            </span>
            <LogoutButton />
          </div>
        </div>
      ...
      <section className="mx-auto max-w-5xl px-6 py-10">
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="border border-line bg-white p-5">
            ...
            <p className="mt-1 font-display text-2xl text-rupee">
          ...
          <div className="border border-line bg-white p-5">
            ...
            <p className="mt-1 font-display text-2xl text-ink">{household.baseCurrency}</p>
          </div>
          <div className="border border-line bg-white p-5">
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-2">
              Operational currency
            </p>
            <p className="mt-1 font-display text-2xl text-dirham">{household.operationalCurrency}</p>
          </div>
          <div className="border border-line bg-white p-5">
            <p className="text-xs font-semibold uppercase tracking-wide text-ink-2">
              Goals tracked
            </p>
            <p className="mt-1 font-display text-2xl text-ink">{household.goals.length}</p>
          </div>
        </div>

        {otherCurrencyCount.count > 0 && (
          <p className="mt-4 border border-line bg-paper-2 px-4 py-2 text-xs text-ink-2">
        ...
        <div className="mt-10">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-xl text-ink">By family member</h2>
            <Link href="/accounts" className="text-sm text-span hover:underline">
              Manage holdings →
            </Link>
          </div>
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {memberBreakdowns.map((m) => (
              <div key={m.id} className="border border-line bg-white p-4">
                <p className="mb-2 text-sm font-medium text-ink">{m.name}</p>
                {Object.keys(m.byClass).length === 0 && m.otherCurrencyHoldings.length === 0 ? (
                  <p className="text-xs text-ink-2">No holdings added yet.</p>
                ) : (
        ...
        <div className="mt-10">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-xl text-ink">Allocation: actual vs target</h2>
            <Link href="/allocation" className="text-sm text-span hover:underline">
              Set targets →
            </Link>
          </div>
        ...
        <div className="mt-10">
          <div className="flex items-center justify-between">
            <h2 className="font-display text-xl text-ink">Goals</h2>
            <Link href="/goals" className="text-sm text-span hover:underline">
              Manage goals →
            </Link>
          </div>
          {household.goals.length === 0 ? (
            <p className="mt-3 text-sm text-ink-2">No goals added yet.</p>
          ) : (
        ...
        <div className="mt-10">
          <h2 className="font-display text-xl text-ink">People with access</h2>
          <div className="mt-4 divide-y divide-line border border-line bg-white">
            {household.users.map((u) => (
              <div key={u.id} className="flex items-center justify-between px-4 py-3">
                ...
                <span className="text-xs uppercase tracking-wide text-span">{u.role}</span>
              </div>
            ))}
          </div>
        </div>

        <p className="mt-10 border border-line bg-paper-2 px-4 py-3 text-sm text-ink-2">
