      ...
      <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm transition-all hover:shadow dark:border-slate-800 dark:bg-canvas-card dark:hover:border-cyan-500/40">
          ...
          <div className="mt-3 space-y-2">
            <div className="flex items-baseline justify-between">
              <span className="text-base text-slate-500 dark:text-slate-400">Last 7 days</span>
              <span className="text-2xl font-medium text-slate-900 dark:text-white">{newLast7Days}</span>
            </div>
            <div className="flex items-baseline justify-between">
              <span className="text-base text-slate-500 dark:text-slate-400">Last 30 days</span>
              <span className="text-2xl font-medium text-slate-900 dark:text-white">{newLast30Days}</span>
            </div>
            <div className="flex items-baseline justify-between border-t border-slate-200/80 pt-2 dark:border-slate-800">
              <span className="text-base text-slate-500 dark:text-slate-400">All time</span>
              <span className="text-2xl font-medium text-slate-900 dark:text-white">{totalClients}</span>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200/80 bg-white p-4 shadow-sm transition-all hover:shadow dark:border-slate-800 dark:bg-canvas-card dark:hover:border-cyan-500/40">
          ...
          {topLocations.length === 0 ? (
            <p className="mt-3 text-base text-slate-500 dark:text-slate-400">No clients yet.</p>
          ) : (
            <div className="mt-3 divide-y divide-slate-200/80 dark:divide-slate-800">
              {topLocations.map(([city, count]) => (
                <div key={city} className="flex items-center justify-between py-1.5">
                  <span className="text-base text-slate-900 dark:text-white">{city}</span>
                  <span className="text-base text-slate-500 dark:text-slate-400">{count}</span>
                </div>
              ))}
            </div>
          )}
